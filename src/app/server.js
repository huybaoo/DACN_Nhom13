...
app.post('/api/save-user', async (req, res) => {
    ...
    try {
        ...
        await newUser.save();
        res.status(201).send({ message: 'Người dùng được lưu thành công!' });
    } catch (error) {
        console.error('Lỗi lưu người dùng:', error);
        res.status(500).send('Đã xảy ra lỗi khi lưu người dùng.');
    }
...
app.post('/place-order', async (req, res) => {
    const { user_id, user_name, order_date, total_amount, status, address, note, orderData } = req.body;
...
app.post('/api/orders', async (req, res) => {
    try {
        const { user_id, fullName, phoneNumber, province, district, ward, address, note, products, total_amount } = req.body;
